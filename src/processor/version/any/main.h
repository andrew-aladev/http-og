...
extern const hog_processor_state_fast_t HOG_PROCESSOR_VERSION_ANY_MIN_LENGTH;
extern const hog_processor_state_fast_t HOG_PROCESSOR_VERSION_ANY_MAX_LENGTH;
...
{
  if (state == HOG_PROCESSOR_VERSION_ANY_MAX_LENGTH) {
    HOG_LOG_ERROR("version_any processor exceeded max length");
    return HOG_PROCESSOR_VERSION_ANY_INITIAL_STATE;
  }

  if (!HOG_PROCESSOR_VERSION_ANY_ALLOWED_BYTES[byte]) {
...
inline bool hog_processor_version_any_is_valid(hog_processor_state_fast_t state)
{
